...
CREATE TABLE `collaboratore` (
  ...
  `descrizionePersonale` varchar(45) NOT NULL,
  `telefonoCellulare` varchar(45) DEFAULT NULL,
  `linkFacebook` varchar(45) DEFAULT NULL,
  `linkInstagram` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`idCollaboratore`,`Utente_idUtente`),
  KEY `Agente_idAgente_idx` (`Agente_idAgente`),
  KEY `Utente_idUtente_idx` (`Utente_idUtente`),
  CONSTRAINT `Agente_idAgente` FOREIGN KEY (`Agente_idAgente`) REFERENCES `agente` (`idAgente`),
  CONSTRAINT `Utente_idUtente` FOREIGN KEY (`Utente_idUtente`) REFERENCES `utente` (`idUtente`) ON DELETE CASCADE ON UPDATE CASCADE
